  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 truncate">{meta}</p>
      </div>
      <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 whitespace-nowrap">{time}</span>
      <span className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap', statusStyle)}>
        {statusLabel}
      </span>
    </div>
  );
