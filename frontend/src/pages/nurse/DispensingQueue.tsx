...
export function NurseDispensingQueue() {
  ...
  const dispenseMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await api.patch(`/pharmacy/prescriptions/${id}/dispense`);
      return res.data;
    },
    onSuccess: () => {
      toast.success('Prescription dispensed');
      void queryClient.invalidateQueries({ queryKey: ['prescriptions', 'nurse'] });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Dispensing Queue</h1>
        <p className="text-muted-foreground">Active prescriptions awaiting dispensing</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {isError ? (
            <p className="text-center py-8 text-red-600">Failed to load dispensing queue. Please try again.</p>
          ) : (
            <DataTable
              data={prescriptions}
              columns={columns}
              isLoading={isLoading}
              emptyMessage="No active prescriptions in the queue"
            />
          )}
        </CardContent>
      </Card>
            ...
            {detailLoading ? (
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-2 pt-2">
                  {detailPrescription.status === 'active' && (
                    <Button
                      onClick={() => dispenseMutation.mutate(detailPrescription._id)}
