...
export function AdminDocumentManagement() {
  ...
  const voidMutation = useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      const res = await api.post(`/documents/${id}/void`, { voidReason: reason });
      return res.data;
    },
    onSuccess: () => {
  ...
  const handleVoidSubmit = () => {
    ...
    voidMutation.mutate({ id: voidTargetId, reason: voidReason.trim() });
  };
        ...
        <div className="flex gap-2">
          ...
          {row.status === 'issued' && (
            <Button
              variant="destructive"
              size="sm"
              onClick={() => openVoidDialog(row._id)}
            >
              Void
            </Button>
          )}
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all patient documents</p>
        </div>
          ...
          <CardContent>
            {detailLoading ? (
              ...
              <div className="space-y-5">
                ...
                {detailDocument.voidedAt && (
                  <p className="text-xs text-muted-foreground">
                    Voided at: {new Date(detailDocument.voidedAt).toLocaleString()}
                  </p>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  {detailDocument.status === 'issued' && (
                    <Button
                      variant="destructive"
                      onClick={() => openVoidDialog(detailDocument._id)}
                    >
                      Void Document
                    </Button>
                  )}
      ...
      <Dialog open={voidOpen} onOpenChange={setVoidOpen}>
        <DialogContent className="max-w-sm" onClose={() => setVoidOpen(false)}>
          <DialogHeader>
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              This action cannot be undone. Please provide a reason for voiding this document.
            </p>
            <div>
