      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="px-4 divide-y divide-slate-50">
            ...
            {!apptQ.isLoading && !apptQ.isError && todayAppts.length === 0 && (
              <p className="text-sm text-slate-400 py-12 text-center">No patients scheduled for today</p>
            )}
          ...
          {/* Footer */}
          <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 text-right">
            <Link to="/doctor/schedule" className="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300">
              Full schedule →
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-50 dark:border-slate-800">
              <h2 className="text-base font-bold text-slate-900 dark:text-white">🧪 Lab Results In</h2>
              <Link to="/doctor/lab" className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">View all →</Link>
            </div>
            <div className="px-6 py-2">
              ...
              {!labQ.isLoading && !labQ.isError && labOrders.length === 0 && (
                <p className="text-sm text-slate-400 py-6 text-center">No recent lab results</p>
              )}
