...
export function PatientDashboard() {
  ...
  const alerts = [
    { dotColor: '#ef4444', text: 'Critical: Infrastructure Data Load High', time: '12m ago' },
    { dotColor: '#22c55e', text: 'Secure Connection Established: VPN-01', time: '2h ago' },
    { dotColor: '#f59e0b', text: 'System Update Scheduled: v2.4.1', time: 'Now' },
  ].slice(0, 3);
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{nextApptLabel}</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-50 dark:border-slate-800">
            <h2 className="text-base font-bold text-slate-900 dark:text-white">My Appointments</h2>
            <Link to="/patient/appointments/new" className="text-sm font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">Book new →</Link>
          </div>
          <div className="px-4 divide-y divide-slate-50">
            ...
            {!apptQ.isLoading && upcomingAppts.length === 0 && (
              <div className="py-12 text-center">
                <p className="text-sm text-slate-400 mb-4">No upcoming appointments</p>
                <Link to="/patient/book-appointment" className="text-sm font-semibold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-xl hover:bg-indigo-100">
                  Book Appointment
                </Link>
              </div>
            )}
            {upcomingAppts.map(appt => {
              ...
              const docName = appt.doctor?.userId
                ...
                : 'Doctor TBD';
              return (
                <div key={appt._id} className="flex items-center gap-4 py-4 border-b border-slate-50 dark:border-slate-800 last:border-0 px-6 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <div className={`w-12 h-12 ${dateParts.bg} rounded-xl flex flex-col items-center justify-center flex-shrink-0`}>
                    <span className={`text-lg font-extrabold ${dateParts.text} leading-none`}>{dateParts.day}</span>
                    <span className={`text-[10px] font-bold ${dateParts.text} opacity-70`}>{dateParts.month}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{docName}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-50 dark:border-slate-800">
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Active Prescriptions</h2>
              <Link to="/patient/prescriptions" className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">View all →</Link>
            </div>
            <div className="px-6 py-2">
              {rxQ.isLoading && <div className="h-16 animate-pulse bg-slate-50 rounded-lg m-2" />}
              {!rxQ.isLoading && activePrescriptions.length === 0 && (
                <p className="text-sm text-slate-400 py-6 text-center">No active prescriptions</p>
              )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-50 dark:border-slate-800">
          <h3 className="text-base font-bold text-slate-900 dark:text-white">Health Activity Timeline</h3>
        </div>
        {timeline.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {timeline.map(event => (
              <div key={`${event.kind}-${event._id}`} className="flex items-center gap-4 py-4 px-6 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                <div className={`w-10 h-10 ${event.iconBg} dark:bg-slate-800 rounded-xl flex items-center justify-center text-lg flex-shrink-0`}>
                  {event.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{event.title}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 truncate">{event.sub}</p>
                </div>
                <span className="text-xs text-slate-400 dark:text-slate-500 whitespace-nowrap flex-shrink-0">{event.date}</span>
              </div>
            ))}
          </div>
        )}
