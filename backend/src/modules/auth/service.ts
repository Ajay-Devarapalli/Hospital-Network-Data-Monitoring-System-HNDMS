...
export async function registerUser(
  ...
  // If user doesn't exist (new signup), create one
  if (!user) {
    user = new User({
      email: input.email,
      role: 'patient', // Default role for public signup
    });
  }
  ...
  if (input.dob) user.dob = new Date(input.dob);
  if (input.gender) user.gender = input.gender as any;
  user.isActive = true;
  ...
  try {
    ...
    const existingPatient = await patientModule.Patient.findOne({ userId: user._id });
    if (!existingPatient) {
